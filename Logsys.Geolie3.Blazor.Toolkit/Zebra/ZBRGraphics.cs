...
namespace nsZBRPrinter
{
	public class ZBRGraphics
    {
        ...
        [DllImport("ZBRGraphics.dll", EntryPoint = "ZBRGDIInitGraphicsEx", CharSet = CharSet.Auto,
            SetLastError = true)]
        static extern int ZBRGDIInitGraphicsEx(byte[] strPrinterName, out IntPtr hDC, 
                                               [MarshalAs(UnmanagedType.LPStr)] string jobName, out int jobID, out int err);

        [DllImport("ZBRGraphics.dll", EntryPoint = "ZBRGDICancelJob", CharSet = CharSet.Auto,
            SetLastError = true)]
        static extern int ZBRGDICancelJob([MarshalAsAttribute(UnmanagedType.LPStr)] string printerName, int jobID, out int err);
        ...
        [DllImport("ZBRGraphics.dll",EntryPoint="ZBRGDIDrawTextEx", CharSet=CharSet.Auto, SetLastError=true)]
        public static extern int ZBRGDIDrawTextEx(int x, int y, int angle, int alignment, byte[] text,   byte[] font, int fontSize, int fontStyle, int color, out int err);

        [DllImport("ZBRGraphics.dll", EntryPoint = "ZBRGDIDrawTextRectEx", CharSet = CharSet.Auto, SetLastError = true)]
        public static extern int ZBRGDIDrawTextRectEx(int x, int y, int width, int height, int angle, int alignment, byte[] text, byte[] font, int fontSize, int fontStyle, int color, out int err);
        ...
        public int InitGraphics(byte[] drvName, string jobName, out int jobID, out int errValue)
        {
            return ZBRGDIInitGraphicsEx(drvName, out _hDC, jobName, out jobID, out errValue);
        }

        //cancels the job in the spooler
        public int CancelJob(string printerName, int jobID, out int errValue)
        {
            return ZBRGDICancelJob(printerName, jobID, out errValue);
        }
        ...
        public int DrawTextRectEx(int x, int y, int width, int height, int angle, int alignment, byte[] text, byte[] font, int fontSize, int fontStyle, int color, out int err)
        {
            return ZBRGDIDrawTextRectEx(x, y, width, height, angle, alignment, text, font, fontSize, fontStyle, color, out err);
        }
